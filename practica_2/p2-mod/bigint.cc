 ...
 */
template <size_t Base, class T>
BigInt<Base, T>::BigInt(std::string &number_str) {
  // Si es vacio
  if (number_str.empty()) {
    sign_ = 1;
    digits_.push_back(0);
 ...
 */
template <size_t Base, class T>
BigInt<Base, T>::BigInt(const char *number_ptr) {
 ...
 */
template <size_t Base, class T>
BigInt<Base, T>::BigInt(const std::vector<char> &digits_aux, const int &sign_aux) {
 ...
 */
template <size_t Base, class T>
BigInt<Base, T>::BigInt(const BigInt<Base, T> &number_big) {
 ...
 * @tparam Base
 * @param number_big
 * @return BigInt<Base, T>&
 */
template <size_t Base, class T>
BigInt<Base, T> &BigInt<Base, T>::operator=(const BigInt<Base, T> &number_big) {
  sign_ = number_big.sign_;
  digits_ = number_big.digits_;
  return *this;
}
 ...
 */
template <size_t Base, class T>
BigInt<Base, T> BigInt<Base, T>::SetSign(int sign_aux) {
  sign_ = sign_aux;
  return *this;
}
 ...
 */
template <size_t Base, class T>
BigInt<Base, T> BigInt<Base, T>::SetDigits(std::vector<char> digits_aux) {
  digits_ = digits_aux;
  return *this;
}
 ...
 */
template <size_t Base, class T>
std::vector<char> BigInt<Base, T>::GetDigits() const {
  return digits_;
}
 ...
 */
template <size_t Base, class T>
int BigInt<Base, T>::GetSign() const {
  return sign_;
}
 ...
 */
template <size_t Base, class T>
size_t BigInt<Base, T>::size() const {
  return digits_.size();
}
/**
 ...
 */
template <size_t Base, class T>
char BigInt<Base, T>::operator[](size_t position) const {
 ...
 */
template <size_t Base, class T>
bool BigInt<Base, T>::operator!=(const BigInt<Base, T> &number_y) const {
 ...
 */
template <size_t Base, class T>
bool BigInt<Base, T>::IsZero() const {
 ...
 */
template <size_t Base, class T>
BigInt<Base, T> BigInt<Base, T>::fill_zeros(unsigned number_zero) const {
  std::string number_str = this->to_string();
  number_str.insert(number_str.begin(), number_zero, '0');
  return BigInt<Base, T>(number_str);
}
 ...
 */
template <size_t Base, class T>
BigInt<Base, T> BigInt<Base, T>::Abs() const {
  BigInt<Base, T> number_aux = *this;
 ...
 */
template <size_t Base, class T>
std::string BigInt<Base, T>::to_string() const {
 ...
 */
template <size_t Base, class T>
BigInt<Base, T> BigInt<Base, T>::operator-(const BigInt<Base, T> &number_y) const {
  size_t number_digit_different =
      abs(this->GetDigits().size() - number_y.GetDigits().size());
  BigInt<Base, T> number_y_aux;
  BigInt<Base, T> number_x_aux;
  ...
  if (number_x_aux < number_y_aux) {
    return BigInt<Base, T>(number_y_aux - number_x_aux).SetSign(-1);
  }
  if (number_x_aux.GetSign() == 1 && number_y_aux.GetSign() == -1) {
    return BigInt<Base, T>(number_x_aux + (-number_y_aux));
  ...
  return BigInt<Base, T>(digits_sub, number_x_aux.GetSign());
}
 ...
 */
template <size_t Base, class T>
BigInt<Base, T> BigInt<Base, T>::PushBack(char number_aux) {
 ...
 */
template <size_t Base, class T>
BigInt<Base, T> operator/(const BigInt<Base, T> &number_x,
                       const BigInt<Base, T> &number_y) {
   BigInt<Base, T> a = number_x;
    BigInt<Base, T> b = number_y;
    if(b.IsZero()){
    ...
    if(a == b){
        return BigInt<Base, T>(1);
    }
    long i, lgcat = 0, cc;
    long n = a.GetDigits().size();
    std::vector<char> cat(n, 0);
    BigInt<Base, T> t;
    BigInt<Base, T> aux_base(Base);
    for (i = n - 1; i >= 0 && (t * aux_base) + BigInt<Base, T>(a[i])  < b; i--){
        t = t * aux_base;
        t = t + BigInt<Base, T>((int)a[i]);
    }
    for (; i >= 0; i--){
        t = (t * Base) + BigInt<Base, T>((int)a[i]);
...
template <size_t Base, class T>
BigInt<Base, T> pow(const BigInt<Base, T> &number_x, const BigInt<Base, T> &number_y) {
static  BigInt<Base, T> zero;
  static  BigInt<Base, T> one (1);
  static  BigInt<Base, T> two (2);
  if (number_y < zero) {
    return zero;
  }
  if (number_y == zero) {
    ...
    BigInt<Base, T> half = pow(number_x, number_y / two);
    return half * half;
  } else {
    BigInt<Base, T> half = pow(number_x, (number_y - one) / two);
...
template <size_t Base, class T>
BigInt<Base, T> BigInt<Base, T>::operator!() const{
  BigInt<Base, T> number_aux = this->Abs();
  BigInt<Base, T> result("1");
  BigInt<Base, T> i("1");
  BigInt<Base, T> zero("0");
  if(number_aux == zero){
    return result;
  }
  for(; i <= number_aux; ++i){
 ...
 */
template <size_t Base, class T>
 BigInt<Base, T>::operator BigInt<2>(){
  BigInt<Base, T> number_aux = this->Abs();
  BigInt<Base, T> zero;
  BigInt<Base> two(2);
